        ...
        <el-button-group>
          <el-button :disabled="changeDisabled">
            ...
            <input
              ...
              :accept="accept"
              onclick="f.outerHTML=f.outerHTML"
          ...
          <div class="item-chunk-box">
            <el-table :data="item.chunkList" border max-height="300">
              ...
              <el-table-column prop="uploaded" label="是否完成" align="center">
                <template v-slot="{ row }">{{ row.uploaded ? '完成' : '进行中' }}</template>
              </el-table-column>
...
<script>
// import { saveObjArr, getObjArr, clearLocalStorage } from "src/utils/localstorage.js"
...
.simple-upload-container .total-progress .select-file-input {
  position: absolute;
...
.file-list :hover .item-chunk-box {
  display: block;
}
.file-list div {
  flex: 1;
...
.file-list .item-status .el-icon-circle-check {
  color: #67c23a;
}
.file-list .el-icon-circle-close {
  color: #f00;
}

.file-list .item-chunk-box {
  display: none;
  transition: all 3s;
  position: absolute;
  top: 0;
...
.el-progress.el-collapse-item__header {
  height: auto;
}
</style>
